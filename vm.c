	...
	for(;;){
		...
		switch(IP->op){
			...
			case OP_LOAD_F:
				...
				break;
			case OP_STORE_F:
				{double fVal=popf();v=popv();*(double*)v.p=fVal;pushf(fVal);
				printf("STORE.f\t// *(double*)%p=%g",v.p,fVal);}
				IP=IP->next;
				break;
			case OP_ADD_F:
				{double b=popf(),a=popf();pushf(a+b);printf("ADD.f\t// %g+%g -> %g",a,b,a+b);}
				IP=IP->next;
				break;
			case OP_SUB_F:
				{double b=popf(),a=popf();pushf(a-b);printf("SUB.f\t// %g-%g -> %g",a,b,a-b);}
				IP=IP->next;
				break;
			case OP_MUL_F:
				{double b=popf(),a=popf();pushf(a*b);printf("MUL.f\t// %g*%g -> %g",a,b,a*b);}
				IP=IP->next;
				break;
			case OP_DIV_I:
				iTop=popi();
				...
				break;
			case OP_DIV_F:
				{double b=popf(),a=popf();pushf(a/b);printf("DIV.f\t// %g/%g -> %g",a,b,a/b);}
				IP=IP->next;
				break;
			case OP_LESS_F:
				{double b=popf(),a=popf();pushi(a<b);printf("LESS.f\t// %g<%g -> %d",a,b,a<b?1:0);}
				IP=IP->next;
				break;
